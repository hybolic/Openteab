...
from openteab.main.lib.kbm_base import MouseBase, KeyboardBase
...
class Fishing(MACRO_BASE):
    ...
    def _run_pre_fishing_sequence(self) -> bool:
        ...
        if collections_x > 0:
            MouseBase.Mouse_Click("left", collections_x, collections_y, speed=3)
        if not self.sleep_interruptible(1.0 + fishing_actions_delay):
            return False

        exit_x, exit_y = self.cfg["exit_collections_button"]
        if exit_x > 0:
            MouseBase.Mouse_Click("left", exit_x, exit_y, speed=3)
        if not self.sleep_interruptible(0.2 + fishing_actions_delay):
            return False
    ...
    def _run_equip_aura_before_movement(self) -> bool:
        ...
        step_delay = float(self.cfg.get("fishing_movement_aura_delay_seconds", 0.67)) + _get_fishing_actions_delay_seconds(self.cfg)
        ...
        if aura_menu_x > 0:
            MouseBase.Mouse_Click("left", aura_menu_x, aura_menu_y, 1, speed=3)
        if not self.sleep_interruptible(step_delay):
            return False

        if aura_search_x > 0:
            MouseBase.Mouse_Click("left", aura_search_x, aura_search_y, 1, speed=3)
        if not self.sleep_interruptible(step_delay):
            return False

        try:
            KeyboardBase.Safe_Type(aura_name, self.cfg)
        except Exception:
            pass
        if not self.sleep_interruptible(step_delay):
            return False
        ...
        if first_slot_x > 0:
            MouseBase.Mouse_Move(first_slot_x, first_slot_y, speed=3)
            if not self.sleep_interruptible(step_delay): return False
            try:
                MouseBase.Mouse_Wheel("up", max(1, int(round(5000 / 120.0))))
            except Exception:
                pass
            if not self.sleep_interruptible(step_delay): return False
            MouseBase.Mouse_Click("left", first_slot_x, first_slot_y, 1, speed=3)


        if not self.sleep_interruptible(step_delay): return False

        if equip_x > 0:
            MouseBase.Mouse_Click("left", equip_x, equip_y, 1, speed=3)
        if not self.sleep_interruptible(step_delay):
            return False

        if close_x > 0:
            MouseBase.Mouse_Click("left", close_x, close_y, 1, speed=3)
            if not self.sleep_interruptible(step_delay):
                return False
    ...
    def _run_recorded_events(self, events: list[dict[str, Any]], speed_multiplier: float = 1.0) -> bool:
        ...
        try:
            for ev in events:
                ...
                try:
                    if typ == "mouse_move":
                        MouseBase.Mouse_Move(int(ev.get("x", 0)), int(ev.get("y", 0)), 0)
                    elif typ == "mouse_down":
                        MouseBase.Mouse_Down(str(ev.get("button", "left")))
                    elif typ == "mouse_up":
                        MouseBase.Mouse_Up(str(ev.get("button", "left")))
                    elif typ == "mouse_wheel":
                        delta = int(ev.get("delta", 0))
                        if delta != 0:
                            MouseBase.Mouse_Wheel("up" if delta > 0 else "down", abs(delta))
                    elif typ == "key_down":
                        k = str(ev.get("key", "")).lower().strip()
                        if k:
                            KeyboardBase.Send(f"{{{k.upper()} down}}")
                            pressed_keys.add(k)
                    elif typ == "key_up":
                        k = str(ev.get("key", "")).lower().strip()
                        if k:
                            KeyboardBase.Send(f"{{{k.upper()} up}}")
            ...
            for key_name in list(pressed_keys):
                try:
                    KeyboardBase.Send(f"{{{key_name.upper()} up}}")
                except Exception:
                    pass
    ...
    def _run_sell_fish_sequence(self,fish_sell_count:int) -> bool:
        ...
        if dialogue_x > 0:
            MouseBase.Mouse_Click("left", dialogue_x, dialogue_y, 1, speed=3)
        ...
        if shop_x > 0:
            MouseBase.Mouse_Click("left", shop_x, shop_y, 1, speed=3)
        ...
        for _ in range(max(1, int(fish_sell_count))):
            if not self.shouldContinue() or not self.canRun():
                return False
            if sell_tab_x > 0:
                MouseBase.Mouse_Click("left", sell_tab_x, sell_tab_y, 1, speed=3)
            if not self.sleep_interruptible(0.3 + fishing_actions_delay):
                return False
            if first_fish_x > 0:
                MouseBase.Mouse_Click("left", first_fish_x, first_fish_y, 1, speed=3)
            if not self.sleep_interruptible(0.3 + fishing_actions_delay):
                return False
            if sell_x > 0:
                MouseBase.Mouse_Click("left", sell_x, sell_y, 1, speed=3)
            if not self.sleep_interruptible(0.3 + fishing_actions_delay):
                return False
            if confirm_x > 0:
                MouseBase.Mouse_Click("left", confirm_x, confirm_y, 1, speed=3)
            if not self.sleep_interruptible(1.5 + fishing_actions_delay):
                return False

        if close_shop_x > 0:
            MouseBase.Mouse_Click("left", close_shop_x, close_shop_y, 1, speed=3)
        ...
        try:
            while self.shouldContinue():
                ...
                if not self.was_runnable:
                    ...
                    for _ in range(3):
                        MouseBase.Mouse_Click("left", close_x, close_y, speed=3)
                    ...
                    MouseBase.Mouse_Click("left", click_x, click_y, speed=3)
                ...
                MouseBase.Mouse_Click("left", start_x, start_y, speed=3)
                self.last_start_fishing_click_at = time.monotonic()
