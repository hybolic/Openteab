    ...
    try:
        while _should_continue():
            ...
            if not was_runnable:
                ...
                if should_sell:
                    ...
                    if should_use_merchant:
                        print(f"merchant flow triggered after pending selling.",type=log_prefix)
                        merchant_ran, _ = _run_merchant_sequence_with_state()
                        if merchant_ran:
                            fish_caught_since_merchant = 0
                        _persist_runtime_counters()
                        if not merchant_ran:
                            print(f"merchant flow skipped/failed during fishing.",type=log_prefix)
                            
                    if should_use_merchant_ocr and merchant_ocr_check_cb is not None:
                        print(f"merchant OCR triggered after pending selling.",type=log_prefix)
                        try:
                            _set_busy(True)
                            merchant_ocr_check_cb()
                        except Exception as e:
                            print(f"merchant OCR check failure: {e}",type=log_prefix)
                        finally:
                            _set_busy(False)
                    ...
                    if should_use_br_sc:
                        print(f"BR/SC flow triggered after pending selling.",type=log_prefix)
                ...
                if should_use_merchant:
                    ...
                    if should_use_br_sc:
                        print(f"BR/SC flow triggered after pending merchant.",type=log_prefix)
                        br_sc_ran = _run_br_sc_sequence()
                        if br_sc_ran:
                            fish_caught_since_br_sc = 0
                        _persist_runtime_counters()
                ...
                if should_use_merchant_ocr and merchant_ocr_check_cb is not None:
                    ...
                    if should_use_br_sc:
                        print(f"BR/SC flow triggered after merchant OCR.",type=log_prefix)
                        br_sc_ran = _run_br_sc_sequence()
                        if br_sc_ran:
                            fish_caught_since_br_sc = 0
                        _persist_runtime_counters()
            ...
            if bool(cfg.get("fishing_enable_selling", False)):
                ...
                if fish_caught_count >= max(1, sell_after):
                    ...
                    if should_use_merchant:
                        print(f"merchant flow triggered after selling.",type=log_prefix)
                        merchant_ran, _ = _run_merchant_sequence_with_state()
                        if merchant_ran:
                            fish_caught_since_merchant = 0
                        _persist_runtime_counters()
                    ...
                    if should_use_merchant_ocr and merchant_ocr_check_cb is not None:
                        print(f"merchant OCR triggered after selling.",type=log_prefix)
                        try:
                            merchant_ocr_check_cb()
                        except Exception as e:
                            print(f"merchant OCR check failure: {e}",type=log_prefix)
                    ...
                    if should_use_br_sc:
                        print(f"BR/SC flow triggered after selling.",type=log_prefix)
                        br_sc_ran = _run_br_sc_sequence()
                        if br_sc_ran:
                            fish_caught_since_br_sc = 0
                        _persist_runtime_counters()
            ...
            if should_use_merchant:
                print(f"merchant flow triggered after {fish_caught_since_merchant} catches.",type=log_prefix)
                ...
                if should_use_merchant_ocr and merchant_ocr_check_cb is not None:
                    print(f"merchant OCR triggered after merchant flow.",type=log_prefix)
                    try:
                        merchant_ocr_check_cb()
                    except Exception as e:
                        print(f"merchant OCR check failure: {e}",type=log_prefix)
                ...
                if should_use_br_sc:
                    print(f"BR/SC flow triggered after merchant.",type=log_prefix)
                    br_sc_ran = _run_br_sc_sequence()
                    if br_sc_ran:
                        fish_caught_since_br_sc = 0
            ...
            if should_use_merchant_ocr and merchant_ocr_check_cb is not None:
                print(f"merchant OCR triggered after {fish_caught_since_merchant_ocr} catches.",type=log_prefix)
                try:
                    merchant_ocr_check_cb()
                except Exception as e:
                    print(f"merchant OCR check failure: {e}",type=log_prefix)
            ...
            if should_use_br_sc:
                print(f"BR/SC flow triggered after {fish_caught_since_br_sc} catches.",type=log_prefix)
                br_sc_ran = _run_br_sc_sequence()
                if br_sc_ran:
                    fish_caught_since_br_sc = 0
                _persist_runtime_counters()
        ...
        if sct is not None:
            try:
                ...
                pass
        if print_start_stop:
            print(f"stopped",type=log_prefix)
