...
class DetectionMixin:
    ...
    def take_aura_screenshot_now(self, force=False):
        try:
            ...
            if aura_menu and aura_menu[0]:
                ...
                time.sleep(0.67)
                try:
                    openteab.save_screenshot(f"aura_screenshot_{int(time.time())}.png",self.send_aura_screenshot_webhook,None)
    ...
    def open_customize_biome_embed(self):
        ...
        def save_and_close():
            ...
            cfg = {}
            try:
                if os.path.exists(openteab.config_json):
                    with open(openteab.config_json, "r", encoding="utf-8") as f:
                        cfg = json.load(f)
            except Exception:
                cfg = {}
            cfg["custom_biome_overrides"] = overrides
            with open(openteab.config_json, "w", encoding="utf-8") as f:
                                ...
                                try:
                                    if getattr(self, "aura_screenshot_var", None) and self.aura_screenshot_var.get() and not self.is_fishing_mode_enabled():
                                        for _ in range(2):
                                            roblox.activate_roblox_window()
                                            time.sleep(0.75)
                                        openteab.save_screenshot(f"aura_{int(time.time())}.png")
                                except Exception as e:
                    ...
                    if match:
                        ...
                        if biome not in self.biome_data:
                            ...
                            self.biome_data[biome] = {
                                "color": "0xffffff",
                                "thumbnail_url": openteab.biome_placeholder
                            }
    ...
    def handle_biome_detection(self, biome, last_biome=None):
        try:
            ...
            if (
                ...
                threading.Thread(target=roblox.terminate_roblox_processes, daemon=True).start()
                return
