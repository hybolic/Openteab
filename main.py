...
from openteab.globals import openteab, roblox, FROM_PATH, join_path
import PreLaunch
...
import logging
import shutil
from openteab.main.core import BiomeTracker
...
##################################
#change to runtime clear web cache option 
...
from openteab.main.config import (
...
def get_frontend_entry_url() -> str:
    #TODO: Move to openteab.globals
    dist_dir   = os.path.join(openteab.frontend, "dist")
    index_file = os.path.join(dist_dir, "index.html")
    if os.path.exists(index_file):
        try:
            cache_bust = int(os.path.getmtime(index_file))
            return f"{index_file}?v={cache_bust}"
        except Exception: return index_file
    else:
...
class Api:
    ...
    def set_window(self, window):
        ...
        if self._calib_mgr is None:
            from openteab.main.base_support import CalibrationManager
    ...
    def get_macro_logs(self, max_lines=100):
        try:
            log_path = join_path(openteab.cwd, "macro_logs.txt")
    ...
    def _on_fishing_failsafe_timeout(self):
        ...
        from openteab.main.base_support import rare_biomes
    ...
    def _start_fishing_worker(self) -> None:
        ...
        with self._fishing_lock:
            ...
            def _run_fishing():
                try:
                    from openteab.main.macros.fishing import run_fishing_loop
                    run_fishing_loop(
